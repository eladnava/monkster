    ...
    return function () {
        ...
        var executeQuery = function (resolve, reject) {
            ...
            var handleErrors = function (err) {
                ...
                if (err && err.message) {
                    // Workaround for extremely rare duplicate key error on _id_ index (ObjectId)
                    if (err.message.includes('duplicate key error') && err.message.includes('index: _id_')) {
                        // Check for _id property in document being inserted and override with new unique ObjectId
